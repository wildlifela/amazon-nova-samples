    ...
    try:
        for item in response:
            if 'audioearningcall_expert' in item:
                source_docs = item['audioearningcall_expert']['raw_response']['source_documents']
                    ...
                    if hasattr(doc, 'page_content'):
                        ...
                        try:
                            ...
                            if content.strip().startswith('{'):
                                ...
                                if all(key in json_obj for key in ['start_timestamp_millis', 'end_timestamp_millis', 'segment_index', 'type', 'text']):
                                    if json_obj['type'] == 'TRANSCRIPT':
                                        timestamps.append({
                                            'start': int(json_obj['start_timestamp_millis']),
                                            'end': int(json_obj['end_timestamp_millis']),
                                            'segment_index': int(json_obj['segment_index']),
                                            'text': json_obj['text'],
                                            'speaker': 'Speaker'
                                        })
                                ...
                                # Case 2: Audio segments array
                                    ...
                                    for segment in json_obj['audio_segments']:
                                        if segment['type'] == 'TRANSCRIPT':
                                            timestamps.append({
                                                'start': int(segment['start_timestamp_millis']),
                                                'end': int(segment['end_timestamp_millis']),
                            ...
                            if '"start_timestamp_millis"' in content and '"type": "TRANSCRIPT"' in content:
                                try:
                                    # Extract values using string operations
                                    start_str = content.split('"start_timestamp_millis": ')[1].split(',')[0]
                                    end_str = content.split('"end_timestamp_millis": ')[1].split(',')[0]
                                    index_str = content.split('"segment_index": ')[1].split(',')[0]
